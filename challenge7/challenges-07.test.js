...
const charCode = (input) => {
  // Solution code here...
  const map1 = input.map(ele => ele.charCodeAt());
  return map1;
};
...
const extractAbilities = (abilities) => {
  // Solution code here...
  const map1 = abilities.map(obj =>obj.ability.name);
...
const extractStats = (stats) => {
  // Solution code here...
  const map1 = stats.map(obj => {
    `name: ${obj.stat.name}`,
    `total: ${obj.stat.baseStat} + ${obj.stat.effort}`
  });
