...
const getBaseStatGreaterThan = (input, minBaseStat) => {
  // Solution code here...
  return input.filter(idx =>{
   const g= idx.baseStat>minBaseStat;
return g;
...
const getStatName = (input, minBaseStat) => {
  // Solution code here...
  const g = getBaseStatGreaterThan(input, minBaseStat)
  return g.map(ele => ele.stat.name)
}
